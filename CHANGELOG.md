...
- Experimental: Bonsai Archive hybrid query strategy. When running with `--data-storage-format=X_BONSAI_ARCHIVE`, historical state queries now use a hybrid read path: recent blocks (within `--Xbonsai-archive-boundary`, default 512) are served from the O(1) Bonsai flat DB layer; older blocks use seekForPrev on archive-keyed `[hash+blockN]` entries. A background migrator automatically converts existing Bonsai flat DB entries to archive format after initial sync completes, writing progress to the DB so it resumes safely after restarts.

...
public class ValidatorDownloaderFactory extends SnapDownloaderFactory {

  private static final Logger LOG = LoggerFactory.getLogger(ValidatorDownloaderFactory.class);

  @SuppressWarnings("UnusedVariable")
  public static Optional<FastSyncDownloader<?>> createValidatorDownloader(
    ...
    if (SyncMode.isFullSync(syncConfig.getSyncMode())) {
      if (fastSyncStateStorage.isFastSyncInProgress()) {
        throw new IllegalStateException(
            "Unable to change the sync mode when snap sync is incomplete, please restart with validator sync mode");
      } else {
    ...
    if (syncState.isResyncNeeded()) {
      ...
      LOG.info(
          "Validator sync was requested, but cannot be enabled because the local blockchain is not empty.");
      return Optional.empty();
    }
    ...
    if (syncState.getCheckpoint().isEmpty()) {
      ...
      if (!syncState.isResyncNeeded()) {
        LOG.info(
            "Validator sync start with block {} and hash {}",

...
/**
 * Peer task for retrieving transaction receipts from peers with minimal decoding.
 *
 * <p>This task is similar to {@link GetReceiptsFromPeerTask} but returns {@link
 * SyncTransactionReceipt} objects which are minimally decoded, reducing memory usage during sync.
 */
    ...
    if (!blockHeaders.isEmpty()) {
      ...
      for (final List<SyncTransactionReceipt> receiptsInBlock : receiptsByBlock) {
        final List<BlockHeader> blockHeaders =
            headersByReceiptsRoot.get(BodyValidation.syncReceiptsRoot(receiptsInBlock));
